...
struct sPlayerCashTossLimiter
{
    var PlayerController pc;
    var float fLastToss;
};
var private transient array<sPlayerCashTossLimiter> PlayerCashTossLimiters;
...
final static function RegisterAllTraders(Actor game, out array<ShopVolume> ShopList, bool bUsingObjectiveMode)
{
    ...
    {
      // Open all shops if the setting is enabled
...
static function bool CanPlayerTossCash(PlayerController pc)
{
    ...
    local int iLimiterIndex;
    local int i; 
    
    if ( pc == None || pc.PlayerReplicationInfo == None )
        return false;

    fCurrentTime = pc.Level.TimeSeconds;

    // clean up stale PCs
    iCleanIndex = 0;
    for ( i = 0; i < default.PlayerCashTossLimiters.Length; i++ ) 
    {
        if ( default.PlayerCashTossLimiters[i].pc != None ) 
        {
            // keep valid entries and shift them down if needed
            if ( iCleanIndex != i )
                default.PlayerCashTossLimiters[iCleanIndex] = default.PlayerCashTossLimiters[i];

            iCleanIndex++;
        }
    }

    // trim array to remove stale entries
    if ( iCleanIndex < default.PlayerCashTossLimiters.Length )
        default.PlayerCashTossLimiters.Length = iCleanIndex;

    // search for the player in the array
    for ( iLimiterIndex = 0; iLimiterIndex < default.PlayerCashTossLimiters.Length; iLimiterIndex++ ) 
    {
        if ( default.PlayerCashTossLimiters[iLimiterIndex].pc == pc ) 
        {
            ...
            default.PlayerCashTossLimiters[iLimiterIndex].fLastToss = fCurrentTime;
            return true;
        }
    }

    // add new player entry
    iLimiterIndex = default.PlayerCashTossLimiters.Length;
    default.PlayerCashTossLimiters.Length = iLimiterIndex + 1;
    default.PlayerCashTossLimiters[iLimiterIndex].pc = pc;
    default.PlayerCashTossLimiters[iLimiterIndex].fLastToss = fCurrentTime;
...
static function ResetCashTossLimiters()
{
    default.PlayerCashTossLimiters.Length = 0;
}
